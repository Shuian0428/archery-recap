window.App = (() => {
  ...
  function computeStats(payload) {
    ...
    const dayMap = payload.day_map || { Day1: [1,2,3,4], Day2: [5,6,7,8], Day3: [9,10,11,12] };
    ...
    // ends (72): 12 rounds * 6 ends; each end is 6 arrows
    const endScores = [];
    const endLabels = [];
    for (let r = 1; r <= 12; r++) {
    ...
    // daily stats
    function dayStats(roundIdxs) {
      const arrows = [];
      for (const r of roundIdxs) arrows.push(...rounds[r - 1]);
    ...
    const day1 = dayStats(dayMap.Day1 || [1,2,3,4]);
    const day2 = dayStats(dayMap.Day2 || [5,6,7,8]);
    const day3 = dayStats(dayMap.Day3 || [9,10,11,12]);

    // ranks from data (if present)
    const roundRanks = Array.isArray(ar["各局序位"])
      ? ar["各局序位"].map(x => (x === null || x === undefined) ? null : Number(x))
      : [];
    const doubleRanks = Array.isArray(ar["雙局序位"])
      ? ar["雙局序位"].map(x => (x === null || x === undefined) ? null : Number(x))
      : [];
  ...
  function renderIndexFromData(data) {
  ...
  async function initAthleteStatic() {
    ...
    if (!id) {
      document.body.innerHTML = "缺少 id（URL 需要 ?id=...）";
      return;
    }
    const res = await fetch(`./data/athletes/${encodeURIComponent(id)}.json`);
    const payload = await res.json();
    renderAthleteFromPayload(payload);
  }

  // ===== Dynamic (optional; for local FastAPI) =====
  ...
  function renderAthleteFromPayload(payload) {
    const stats = computeStats(payload);
    const avg = payload.group_avg || {};
    const finalRank = payload.final_rank ?? payload.archer?.["最後名次"] ?? payload.archer?.final_rank ?? "-";

    const sub = document.getElementById("sub");
    if (sub) {
      sub.textContent = `${payload.name}｜${payload.unit || ""}｜${payload.group}｜總名次：${finalRank}`;
    }
    ...
    if (distBars) {
      ...
      const bars = [
        { label: "X",        val: stats.counts.X,        color: "rgba(245,158,11,.95)" },
        { label: "10",       val: stats.counts["10"],    color: "rgba(245,158,11,.75)" },
        { label: "9",        val: stats.counts["9"],     color: "rgba(245,158,11,.55)" },
        { label: "紅圈(7&8)", val: stats.counts.RED,     color: "rgba(239,68,68,.75)" },
        { label: "藍圈(5&6)", val: stats.counts.BLUE,    color: "rgba(59,130,246,.75)" },
      ];
    ...
    if (d1 && d2 && d3) {
      ...
      for (const [t, v] of dayCells(stats.day1)) d1.appendChild(dayItem(t, v));
      for (const [t, v] of dayCells(stats.day2)) d2.appendChild(dayItem(t, v));
      for (const [t, v] of dayCells(stats.day3)) d3.appendChild(dayItem(t, v));
    }

    // ===== Charts =====
    if (typeof Chart === "undefined") return; // index page 沒 Chart.js

    // 1) round score
    const rMM = arrMinMax(stats.roundScores);
    const roundCanvas = document.getElementById("roundScore");
    if (roundCanvas) {
      makeLineChart(
        roundCanvas,
    ...
    const doubleCanvas = document.getElementById("doubleScore");
    if (doubleCanvas) {
      makeLineChart(
        doubleCanvas,
    ...
    const endCanvas = document.getElementById("endScore");
    if (endCanvas) {
      makeLineChart(
        endCanvas,
    ...
    const rrCanvas = document.getElementById("roundRank");
    if (rrCanvas) {
      new Chart(rrCanvas, {
        type: "line",
        data: {
    ...
    // 6) double rank
    const drCanvas = document.getElementById("doubleRank");
    if (drCanvas) {
      new Chart(drCanvas, {
        type: "line",
        data: {
